...
public final class ItemStack extends net.minecraftforge.common.capabilities.CapabilityProvider<ItemStack> implements net.minecraftforge.common.extensions.IForgeItemStack {
   ...
   /** The entity the item is attached to */
   ...
   private ItemStack(IItemProvider itemIn, int countIn, Optional<CompoundNBT> nbtIn) {
      this(itemIn, countIn);
      nbtIn.ifPresent(this::setTag);
   }
   ...
   /**
    * Calls the delegated method to the Item to damage the incoming Entity
    */
            ...
            for(Entry<Attribute, AttributeModifier> entry : multimap.entries()) {
               ...
               if (playerIn != null) {
                  if (attributemodifier.getID() == Item.ATTACK_DAMAGE_MODIFIER) {
                     d0 = d0 + playerIn.func_233638_c_(Attributes.ATTACK_DAMAGE);
                     d0 = d0 + (double)EnchantmentHelper.getModifierForCreature(this, CreatureAttribute.UNDEFINED);
                     flag = true;
                  } else if (attributemodifier.getID() == Item.ATTACK_SPEED_MODIFIER) {
                     d0 += playerIn.func_233638_c_(Attributes.ATTACK_SPEED);
                     flag = true;
                  }
               }
   ...
   private static Collection<ITextComponent> getPlacementTooltip(String stateString) {
      try {
         ...
         if (flag || flag1) {
            if (flag) {
               return Lists.newArrayList(blockstate.getBlock().getTranslatedName().func_240699_a_(TextFormatting.DARK_GRAY));
            }
   ...
   public void setAttachedEntity(@Nullable Entity entityIn) {
      this.attachedEntity = entityIn;
   }
