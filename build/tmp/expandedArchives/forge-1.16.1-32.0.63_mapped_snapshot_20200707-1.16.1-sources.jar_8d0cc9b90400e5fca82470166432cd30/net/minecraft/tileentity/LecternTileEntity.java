...
public class LecternTileEntity extends TileEntity implements IClearable, INamedContainerProvider {
   ...
   /**
    * Creates a CommandSource for resolving the contents of a book. If the player is null
    */
   ...
   public void read(BlockState stateIn, CompoundNBT nbtIn) {
      ...
      if (nbtIn.contains("Book", 10)) {
         this.book = this.ensureResolved(ItemStack.read(nbtIn.getCompound("Book")), (PlayerEntity)null);
      } else {
         this.book = ItemStack.EMPTY;
      }

      this.pages = WrittenBookItem.func_220049_j(this.book);
      this.page = MathHelper.clamp(nbtIn.getInt("Page"), 0, this.pages - 1);
   }
