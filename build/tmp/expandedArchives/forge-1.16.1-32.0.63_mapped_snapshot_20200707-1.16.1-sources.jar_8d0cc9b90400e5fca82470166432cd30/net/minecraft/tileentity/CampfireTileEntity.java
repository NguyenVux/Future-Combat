...
public class CampfireTileEntity extends TileEntity implements IClearable, ITickableTileEntity {
   ...
   /**
    * Individually tracks the cooking of each item
    */
   ...
   public void read(BlockState stateIn, CompoundNBT nbtIn) {
      ...
      ItemStackHelper.loadAllItems(nbtIn, this.inventory);
      if (nbtIn.contains("CookingTimes", 11)) {
         int[] aint = nbtIn.getIntArray("CookingTimes");
         System.arraycopy(aint, 0, this.cookingTimes, 0, Math.min(this.cookingTotalTimes.length, aint.length));
      }

      if (nbtIn.contains("CookingTotalTimes", 11)) {
         int[] aint1 = nbtIn.getIntArray("CookingTotalTimes");
