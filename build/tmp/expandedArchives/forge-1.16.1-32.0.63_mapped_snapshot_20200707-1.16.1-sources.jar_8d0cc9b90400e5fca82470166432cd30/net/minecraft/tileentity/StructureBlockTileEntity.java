...
public class StructureBlockTileEntity extends TileEntity {
   ...
   public void read(BlockState stateIn, CompoundNBT nbtIn) {
      ...
      this.setName(nbtIn.getString("name"));
      this.author = nbtIn.getString("author");
      this.metadata = nbtIn.getString("metadata");
      int i = MathHelper.clamp(nbtIn.getInt("posX"), -48, 48);
      int j = MathHelper.clamp(nbtIn.getInt("posY"), -48, 48);
      int k = MathHelper.clamp(nbtIn.getInt("posZ"), -48, 48);
      this.position = new BlockPos(i, j, k);
      int l = MathHelper.clamp(nbtIn.getInt("sizeX"), 0, 48);
      int i1 = MathHelper.clamp(nbtIn.getInt("sizeY"), 0, 48);
      int j1 = MathHelper.clamp(nbtIn.getInt("sizeZ"), 0, 48);
      this.size = new BlockPos(l, i1, j1);

      try {
         this.rotation = Rotation.valueOf(nbtIn.getString("rotation"));
      } catch (IllegalArgumentException illegalargumentexception2) {
         this.rotation = Rotation.NONE;
      }

      try {
         this.mirror = Mirror.valueOf(nbtIn.getString("mirror"));
      } catch (IllegalArgumentException illegalargumentexception1) {
         this.mirror = Mirror.NONE;
      }

      try {
         this.mode = StructureMode.valueOf(nbtIn.getString("mode"));
      } catch (IllegalArgumentException illegalargumentexception) {
         this.mode = StructureMode.DATA;
      }

      this.ignoreEntities = nbtIn.getBoolean("ignoreEntities");
      this.powered = nbtIn.getBoolean("powered");
      this.showAir = nbtIn.getBoolean("showair");
      this.showBoundingBox = nbtIn.getBoolean("showboundingbox");
      if (nbtIn.contains("integrity")) {
         this.integrity = nbtIn.getFloat("integrity");
      } else {
         this.integrity = 1.0F;
      }

      this.seed = nbtIn.getLong("seed");
