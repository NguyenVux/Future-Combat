...
public class PistonTileEntity extends TileEntity implements ITickableTileEntity {
   ...
   public void read(BlockState stateIn, CompoundNBT nbtIn) {
      ...
      this.pistonState = NBTUtil.readBlockState(nbtIn.getCompound("blockState"));
      this.pistonFacing = Direction.byIndex(nbtIn.getInt("facing"));
      this.progress = nbtIn.getFloat("progress");
      this.lastProgress = this.progress;
      this.extending = nbtIn.getBoolean("extending");
      this.shouldHeadBeRendered = nbtIn.getBoolean("source");
   }
