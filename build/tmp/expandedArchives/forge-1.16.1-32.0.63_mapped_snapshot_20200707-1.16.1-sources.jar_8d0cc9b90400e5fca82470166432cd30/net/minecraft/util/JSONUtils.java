...
public class JSONUtils {
    ...
    */
   public static long getLongValidated(JsonElement jsonIn, String memberNameIn) {
      if (jsonIn.isJsonPrimitive() && jsonIn.getAsJsonPrimitive().isNumber()) {
         return jsonIn.getAsLong();
      } else {
         throw new JsonSyntaxException("Expected " + memberNameIn + " to be a Long, was " + toString(jsonIn));
      }
   }

   /**
    * Gets a long from a JSON element
    */
   public static long getLong(JsonObject jsonIn, String memberNameIn) {
      if (jsonIn.has(memberNameIn)) {
         return getLongValidated(jsonIn.get(memberNameIn), memberNameIn);
      } else {
         throw new JsonSyntaxException("Missing " + memberNameIn + ", expected to find a Long");
      }
   }

   public static long getLong(JsonObject jsonIn, String memberNameIn, long fallbackIn) {
      return jsonIn.has(memberNameIn) ? getLongValidated(jsonIn.get(memberNameIn), memberNameIn) : fallbackIn;
   }
   ...
   public static byte getByte(JsonObject jsonIn, String memberNameIn, byte fallbackIn) {
      return jsonIn.has(memberNameIn) ? getByte(jsonIn.get(memberNameIn), memberNameIn) : fallbackIn;
   }
   ...
   /**
    * Gets the JsonObject field on the JsonObject with the given name
    */
   ...
   @Nullable
   public static <T> T fromJson(Gson gsonIn, Reader readerIn, TypeToken<T> typeIn, boolean lenientIn) {
      try {
         JsonReader jsonreader = new JsonReader(readerIn);
         jsonreader.setLenient(lenientIn);
         return gsonIn.getAdapter(typeIn).read(jsonreader);
      } catch (IOException ioexception) {
         throw new JsonParseException(ioexception);
      }
   }
   ...
   @Nullable
   @OnlyIn(Dist.CLIENT)
   public static <T> T fromJson(Gson gsonIn, String stringIn, TypeToken<T> typeIn, boolean lenientIn) {
      return fromJson(gsonIn, new StringReader(stringIn), typeIn, lenientIn);
   }
   ...
   @Nullable
   public static <T> T fromJsonUnlenient(Gson gsonIn, Reader readerIn, TypeToken<T> typeIn) {
      return fromJson(gsonIn, readerIn, typeIn, false);
   }

   @Nullable
   @OnlyIn(Dist.CLIENT)
   public static <T> T fromJsonUnlenient(Gson gsonIn, String stringIn, TypeToken<T> typeIn) {
      return fromJson(gsonIn, stringIn, typeIn, false);
   }
   ...
   public static JsonObject fromJson(String stringIn, boolean lenientIn) {
      return fromJson(new StringReader(stringIn), lenientIn);
   }

   public static JsonObject fromJson(Reader readerIn, boolean lenientIn) {
      return fromJson(field_212747_a, readerIn, JsonObject.class, lenientIn);
   }

   public static JsonObject fromJson(String stringIn) {
      return fromJson(stringIn, false);
   }
