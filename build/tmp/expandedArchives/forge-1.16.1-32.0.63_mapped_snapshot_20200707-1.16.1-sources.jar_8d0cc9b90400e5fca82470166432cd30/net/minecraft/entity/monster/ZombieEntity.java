...
public class ZombieEntity extends MonsterEntity {
   ...
   public static AttributeModifierMap.MutableAttribute func_234342_eQ_() {
      return MonsterEntity.func_234295_eP_().func_233815_a_(Attributes.FOLLOW_RANGE, 35.0D).func_233815_a_(Attributes.MOVEMENT_SPEED, (double)0.23F).func_233815_a_(Attributes.ATTACK_DAMAGE, 3.0D).func_233815_a_(Attributes.ARMOR, 2.0D).func_233814_a_(Attributes.ZOMBIE_SPAWN_REINFORCEMENTS);
   }
            ...
            for(int l = 0; l < 50; ++l) {
               ...
               if (WorldEntitySpawner.canCreatureTypeSpawnAtLocation(entityspawnplacementregistry$placementtype, this.world, blockpos, entitytype) && EntitySpawnPlacementRegistry.func_223515_a(entitytype, this.world, SpawnReason.REINFORCEMENT, blockpos, this.world.rand)) {
                  ...
                  if (!this.world.isPlayerWithin((double)i1, (double)j1, (double)k1, 7.0D) && this.world.checkNoEntityCollision(zombieentity) && this.world.hasNoCollisions(zombieentity) && !this.world.containsAnyLiquid(zombieentity.getBoundingBox())) {
                     ...
                     this.getAttribute(Attributes.ZOMBIE_SPAWN_REINFORCEMENTS).func_233769_c_(new AttributeModifier("Zombie reinforcement caller charge", (double)-0.05F, AttributeModifier.Operation.ADDITION));
                     zombieentity.getAttribute(Attributes.ZOMBIE_SPAWN_REINFORCEMENTS).func_233769_c_(new AttributeModifier("Zombie reinforcement callee charge", (double)-0.05F, AttributeModifier.Operation.ADDITION));
                     break;
                  }
               }
            }
   ...
   protected void applyAttributeBonuses(float difficulty) {
      ...
      this.getAttribute(Attributes.KNOCKBACK_RESISTANCE).func_233769_c_(new AttributeModifier("Random spawn bonus", this.rand.nextDouble() * (double)0.05F, AttributeModifier.Operation.ADDITION));
      double d0 = this.rand.nextDouble() * 1.5D * (double)difficulty;
      if (d0 > 1.0D) {
         this.getAttribute(Attributes.FOLLOW_RANGE).func_233769_c_(new AttributeModifier("Random zombie-spawn bonus", d0, AttributeModifier.Operation.MULTIPLY_TOTAL));
      }

      if (this.rand.nextFloat() < difficulty * 0.05F) {
         this.getAttribute(Attributes.ZOMBIE_SPAWN_REINFORCEMENTS).func_233769_c_(new AttributeModifier("Leader zombie bonus", this.rand.nextDouble() * 0.25D + 0.5D, AttributeModifier.Operation.ADDITION));
         this.getAttribute(Attributes.MAX_HEALTH).func_233769_c_(new AttributeModifier("Leader zombie bonus", this.rand.nextDouble() * 3.0D + 1.0D, AttributeModifier.Operation.MULTIPLY_TOTAL));
