...
public abstract class AbstractFireBlock extends Block {
   ...
   public AbstractFireBlock(AbstractBlock.Properties propertiesIn, float fireDamageIn) {
      super(propertiesIn);
   ...
   public static BlockState getFireForPlacement(IBlockReader readerIn, BlockPos posIn) {
      BlockPos blockpos = posIn.down();
      BlockState blockstate = readerIn.getBlockState(blockpos);
      return SoulFireBlock.func_235577_c_(blockstate.getBlock()) ? Blocks.SOUL_FIRE.getDefaultState() : ((FireBlock)Blocks.FIRE).getStateForPlacement(readerIn, posIn);
   }
   ...
   protected abstract boolean canBurn(BlockState stateIn);
   ...
   public static boolean canLightBlock(IWorld worldIn, BlockPos posIn) {
      BlockState blockstate = worldIn.getBlockState(posIn);
      BlockState blockstate1 = getFireForPlacement(worldIn, posIn);
      return blockstate.isAir() && (blockstate1.isValidPosition(worldIn, posIn) || shouldLightPortal(worldIn, posIn));
   }

   private static boolean shouldLightPortal(IWorld worldIn, BlockPos posIn) {
      for(Direction direction : Direction.Plane.HORIZONTAL) {
         if (worldIn.getBlockState(posIn.offset(direction)).isIn(Blocks.OBSIDIAN) && NetherPortalBlock.isPortal(worldIn, posIn) != null) {
            return true;
         }
      }
